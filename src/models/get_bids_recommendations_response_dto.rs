/*
 * Партнерский API Маркета
 *
 * API Яндекс Маркета помогает продавцам автоматизировать и упростить работу с маркетплейсом.  В числе возможностей интеграции:  * управление каталогом товаров и витриной,  * обработка заказов,  * изменение настроек магазина,  * получение отчетов.
 *
 * The version of the OpenAPI document: LATEST
 *
 * Generated by: https://openapi-generator.tech
 */

/// GetBidsRecommendationsResponseDto : Список товаров с рекомендованными ставками.

#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct GetBidsRecommendationsResponseDto {
    /// Список товаров с рекомендованными ставками.
    #[serde(rename = "recommendations")]
    pub recommendations: Vec<crate::models::SkuBidRecommendationItemDto>,
}

impl GetBidsRecommendationsResponseDto {
    /// Список товаров с рекомендованными ставками.
    pub fn new(
        recommendations: Vec<crate::models::SkuBidRecommendationItemDto>,
    ) -> GetBidsRecommendationsResponseDto {
        GetBidsRecommendationsResponseDto { recommendations }
    }
}
